...
void* nolock_malloc(size_t bytes, bool do_hash);
void nolock_free(void* p, bool do_hash);
...
void undo_slab (Header* slab, void* p) {
    ...
    if (slb->bitmap == 0) { // if this is true, then we need to remove slb from its slablist
		...
		nolock_free(ptr_add(slab, 4), true); // same thing as before with free issue maybe
	        return;
	    }
	    ...
	    nolock_free(ptr_add(slab, 4), true); // since the node is removed from the free list, we can just remove it from the heap by using free() <-- expects 4+ pt
	    return; 
        }
    }
}
...
small_node** normal_pointers;

inline int hash (void* x) {
    return (((uint32_t)x) / 4) % NP_SIZE; // divide by 4 because pointers are 4B aligned
}

void add_to_normal_pointers (void* x) {
//    ASSERT(normal_pointers != nullptr); 
    if (normal_pointers == nullptr) return; // design choice to not have normal_pointers be considered one of the normal_pointers
    int index = hash(x);
    small_node* ptr = normal_pointers[index];
    
    while (ptr != nullptr) {

        for (int i = 0; i < 8; i++) {
	    if (ptr->data[i] == nullptr) { // empty spot found
	        ptr->data[i] = x;
		return;
	    }
	}
    ...
    //do_hash is false because we don't want to store the pointer from the new node into the hash table
    small_node* p = (small_node*) nolock_malloc(sizeof(small_node), false); // issue... thinks it should be in a slab... also issue of recursive malloc :(
    p->data[0] = x;
    
    for (int i = 1; i < 8; i++) {
        p->data[i] = nullptr; // needs to be zeroed out
    }
    p->next = normal_pointers[index];
    if (p->next != nullptr) {
        p->next->prev = p; // set the older node's back pointer
    }
    p->prev = nullptr;
    normal_pointers[index] = p;
}

bool all_empty (void** arr) {
...
bool remove_from_normal_pointers (void* x) {
    int index = hash(x);
    small_node* ptr = normal_pointers[index];
    while (ptr != nullptr) {

        for (int i = 0; i < 8; i++) {
            if (ptr->data[i] == x) { // x found
                ptr->data[i] = nullptr;
                
                if (all_empty(ptr->data)) {
		    if (ptr->prev == nullptr) { // first node case
		        normal_pointers[index] = normal_pointers[index]->next;
			if (normal_pointers[index] != nullptr) {
			    normal_pointers[index]->prev = nullptr; // need to update its prev
			}
                        nolock_free(ptr, false);
		    } else {
		        ptr->prev->next = ptr->next;
		        if (ptr->next != nullptr) { // update new prev pointer of ptr->next
		            ptr->next->prev = ptr->prev;
		        }
		        nolock_free(ptr, false);
		    }
		}
		return true; // because x was found
            }
        }
	ptr = ptr->next;
    }
    // otherwise, x not found
    return false;
}

void init_normal_pointers() {
...
void* nolock_malloc(size_t bytes, bool do_hash) { //using best fit policy
...
//    }
    ...
    if (abs(best_fit_node->size_and_state) >= (ssize_t)(MIN_BLOCK_SIZE + bytes + 8)) { // leftover header & footer from old node + min_block_size needed for new free n
        ...
        //sanity_checker();
        //print_heap();
	if (do_hash) add_to_normal_pointers(new_header->get_block()); // hhmmmmmm
        ...
        //sanity_checker();
        //print_heap();
	if (do_hash) add_to_normal_pointers(best_fit_node->get_block());
...
void* malloc(size_t bytes) { //using best fit policy
    ...
    if (bytes < MIN_BLOCK_SIZE) { // making slab nodes for malloc(4), malloc(8), malloc(12), malloc(16)  cases... for now
        return do_slab(bytes);
    }
    if (bytes > (heap_size - NODE_OVERHEAD)) { // because heap_size is actually an overestimation already (since header + footer)
        return nullptr;
	...
	sanity_checker();
        //print_heap();
	add_to_normal_pointers(new_header->get_block());
        return new_header->get_block(); // since new_header is pointing to the allocated portion
    }
    else { // case where exact fit or 4/8/12/16 extra bytes more than exact fit
	best_fit_node -> size_and_state *= -1; //indicate that it's now full
	...
	sanity_checker();
        //print_heap();
	add_to_normal_pointers(best_fit_node->get_block());
        return best_fit_node->get_block();
    }
}
...
void nolock_free(void* p, bool do_hash) {
    ...
    bool is_normal = (!do_hash) ? true : remove_from_normal_pointers(p); // if do_hash is false, then we know it's normal node (i.e. not slab)
    if (! is_normal) {
    ...
    Header* right_node = (node -> get_right_header() >= heap_end) ? nullptr : node->get_right_header(); // heap_end is a multiple of 4, we can't have a right node star
